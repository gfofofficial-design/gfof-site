...
function render(){const personal=state.mode==="photo";$("character-controls").hidden=personal;$("photo-controls").hidden=!personal;$("shuffle").hidden=personal;$("portrait-mode").value=state.mode||"character";$("portrait").innerHTML=personal?(state.photo?`<img src="${state.photo}" alt="Your chosen personal portrait">`:'<p>Choose an image to preview your portrait.</p>'):svg();$("preview-name").textContent=state.callsign;$("preview-description").textContent=personal?"Your image · Your Journey":`${state.form.charAt(0).toUpperCase()+state.form.slice(1)} · ${state.accent} suit accent`; }
...
fill();
$("avatar-form").addEventListener("input",event=>{if(event.target&&event.target.id==="photo-file")return;if(event.target&&event.target.id==="portrait-mode"){photoGeneration++;photoLoading=false;}state=clean({...state,...Object.fromEntries(Object.keys(defaults).map(k=>[k,$(k).value])),mode:$("portrait-mode").value});if(event.target&&["photo-zoom","photo-x","photo-y"].includes(event.target.id)){try{state.photo=crop();}catch{$("avatar-status").textContent="This crop could not be created. Try another image.";return;}}render();$("avatar-status").textContent="Preview updated. Save to keep these choices on this device.";});
$("avatar-form").addEventListener("submit",event=>{event.preventDefault();if(photoLoading||state.mode==="photo"&&!state.photo){$("avatar-status").textContent="Choose an image and wait for its preview before saving.";return;}try{localStorage.setItem(key,JSON.stringify({...state,photo:state.mode==="photo"?state.photo:null}));$("avatar-status").textContent="Saved on this device. Return here to edit your avatar.";}catch{$("avatar-status").textContent="This browser could not save your avatar. You can still download the portrait.";}});
...
$("forget").addEventListener("click",()=>{try{localStorage.removeItem(key);photoGeneration++;photoLoading=false;originalImage=null;$("crop-controls").disabled=true;$("photo-file").value="";state={...defaults,mode:"character",photo:null};fill();$("avatar-status").textContent="Saved avatar removed from this browser. Mission badges are unchanged.";}catch{$("avatar-status").textContent="This browser could not remove the saved avatar. Use its site-data settings to remove it.";}});
...
$("download-png").addEventListener("click",async()=>{
 if(photoLoading||state.mode==="photo"&&!state.photo){$("avatar-status").textContent="Choose an image and wait for its preview before downloading.";return;}
 ...
 try{
  ...
  const source=state.mode==="photo"?state.photo:(sourceUrl=URL.createObjectURL(new Blob([svg()],{type:"image/svg+xml"})));
  ...
  const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download="federation-avatar.png";
  try{document.body.append(link);link.click();}finally{link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
  $("avatar-status").textContent="PNG download requested. Your 1024 × 1024 portrait contains no callsign. Nothing was uploaded.";
 }catch{$("avatar-status").textContent="This browser could not create the PNG. Try Download original format instead.";}
 finally{if(sourceUrl)URL.revokeObjectURL(sourceUrl);button.disabled=false;}
});
$("download").addEventListener("click",()=>{if(photoLoading||state.mode==="photo"&&!state.photo){$("avatar-status").textContent="Choose an image and wait for its preview before downloading.";return;}if(state.mode==="photo"){const a=document.createElement("a");a.href=state.photo;a.download="federation-portrait.jpg";document.body.append(a);a.click();a.remove();$("avatar-status").textContent="Portrait download requested. Your callsign is not included in the image.";return;}const url=URL.createObjectURL(new Blob([svg()],{type:"image/svg+xml"}));const a=document.createElement("a");a.href=url;a.download="federation-explorer.svg";document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);$("avatar-status").textContent="Portrait download requested. Your callsign is not included in the image.";});
})();
