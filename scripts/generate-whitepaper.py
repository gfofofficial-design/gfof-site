...
pdfmetrics.registerFont(TTFont("FederationSans", FONT_DIR / "Vera.ttf"))
pdfmetrics.registerFont(TTFont("FederationSans-Bold", FONT_DIR / "VeraBd.ttf"))

styles = getSampleStyleSheet()
title = ParagraphStyle("Title", parent=styles["Title"], fontName="FederationSans-Bold", fontSize=29, leading=31, textColor=TEXT, alignment=TA_CENTER, spaceAfter=16)
h1 = ParagraphStyle("H1", parent=styles["Heading1"], fontName="FederationSans-Bold", fontSize=20, leading=24, textColor=GOLD, spaceAfter=12)
h2 = ParagraphStyle("H2", parent=styles["Heading2"], fontName="FederationSans-Bold", fontSize=12, leading=15, textColor=CYAN, spaceBefore=8, spaceAfter=5)
body = ParagraphStyle("Body", parent=styles["BodyText"], fontName="FederationSans", fontSize=9.4, leading=14, textColor=TEXT, spaceAfter=7)
small = ParagraphStyle("Small", parent=body, fontSize=7.8, leading=11, textColor=MUTED)
tag = ParagraphStyle("Tag", parent=small, fontName="FederationSans", alignment=TA_CENTER, textColor=GREEN, spaceAfter=12)
...
def footer(canvas, doc):
    ...
    canvas.setFont("FederationSans",7); canvas.setFillColor(MUTED)
    canvas.drawString(18*mm,10*mm,"GFOF WHITEPAPER v1.2 · 2026-09-17")
...
section("SECTION 03", "Token and treasury", ["$GFOF is an SPL token on Solana with a fixed total supply of 1,000,000,000. The canonical contract address is printed on the cover and must be verified character-by-character.","Distribution is recorded as 77% community and liquidity, 10% rewards and reserve, and 13% developer allocation locked across four active Streamflow contracts. The Treasury page links every contract and independent explorer path.","$GFOF trades through a Meteora Dynamic Bonding Curve. Migration is triggered by the configured 66,040.882531 USDC quote-reserve threshold, not market capitalization, and targets Meteora DAMM v2. Live reserve progress is not published without a canonical source."], [["SUPPLY","1,000,000,000 $GFOF"],["LOCKED","130,000,000 - 13% - four active contracts"],["MIGRATION","METEORA DBC TO DAMM V2"],["VERIFY","galacticfederation.co/treasury"]])
section("SECTION 04", "Governance boundaries", ["The Realms deployment is advisory and gated to the $GFOF mint. The locked 13% developer allocation cannot vote. No migration event transfers treasury, repository, site, or token authority.","One token equals one advisory vote under the published configuration, but voting does not imply custody or execution authority. Any authority change requires a new specification, security review, and recorded authorization."], [["SPEC","galacticfederation.co/governance-spec"],["CURRENT SCOPE","Advisory only"],["AUTHORITY","Not routed through the DAO"]])
...
section("SECTION 06", "Specifications and research", ["Published design documents cover governance, borrower-protective liquidation, access boundaries, and creator-fee routing. They are historical design artifacts, not proof of implementation or launch authorization.","The prior venue-specific access routing and creator-fee mechanism are superseded and inactive. Corrected specifications, review, and recorded authorization are required before activation.","Post-quantum work is research only. No cryptographic migration is planned and no quantum-security claim is made."], [["LIQUIDATION","galacticfederation.co/liquidation-spec"],["ACCESS","SUPERSEDED IN PART"],["CREATOR FEES","ON HOLD"],["POST-QUANTUM","galacticfederation.co/faq#quantum"]])
