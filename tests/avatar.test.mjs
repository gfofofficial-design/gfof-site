...
function boot(saved=null,blocked=false,imageOptions={}){
 ...
 vm.runInNewContext(model+code,{document:{getElementById:get,createElement:tag=>tag==="canvas"?{getContext(){return {fillRect(){},drawImage(...args){drawArgs=args;}};},toDataURL(){return "data:image/jpeg;base64,YQ==";}}:element(),body:{append(){}}},localStorage:storage,Image:class{naturalWidth=imageOptions.width||800;naturalHeight=imageOptions.height||400;decode(){return imageOptions.decode?imageOptions.decode():Promise.resolve();}},Blob,URL:{createObjectURL(b){blob=b;return 'blob:test';},revokeObjectURL(){}},setTimeout(f){f();},Math});
